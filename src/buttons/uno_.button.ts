...
export default class Uno extends Button {
    ...
    async run(interaction: ButtonInteraction) {
        ...
        if (option === "jn") {
            if (!uno.players.has(interaction.user.id)) uno.emit("join", new Player(interaction.user.id));
            ...
            if (interaction.user.id === uno.host.id) {
                ...
                interaction.reply({
                    content: "Solo el host puede iniciar la partida",
                    ephemeral: true,
                });
            } else
                interaction.reply({
                    content: "Debes unirte a la partida primero",
                    ephemeral: true,
                });
        } else if (option === "mc") {            
            ...
            uno.emit('play', player, player?.cards.find(c=>c.id == option), interaction)
        }
    }
}
