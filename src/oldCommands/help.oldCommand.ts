        ...
        try {
            ...
            if (args?.length) {
                ...
                if (!req.ok) general = true
                    ...
                    embed.addField(
                        'Use:',
                        `\`\`\`\n${res.type == 'slash' ? '/' : server?.getPrefixes(true)[0] ?? server?.prefixes[0]}${
                            res.use
                        }\n\`\`\``,
                        true
                    )
                    ...
                    if (res.user_permisions.length > 0)
                        embed.addField('Permissions required:', `\`${res.user_permisions.join('` `')}\``, true)
                    embed.setFooter({
                            ...
                            .then(async cmds => {
                                ...
                                for (const i of res as string[]) {
                                    const btn = new MessageButton()
                                        .setStyle(i == (res as string[])[0] ? 'SUCCESS' : 'PRIMARY')
                                        .setLabel(i)
