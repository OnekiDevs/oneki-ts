...
export default class Poll extends Command {
    ...
    async make(interaction: CommandInteraction): Promise<any> {
        ...
        if (!server?.premium && !snap.empty)
            return interaction.editReply(`Solo se puede tener una encuesta a la vez. 
            Actualize su membresia a Premium o finalize la encuesta existente`);
        if (!checkSend(interaction.channel as TextChannel, interaction.guild?.me!)) return interaction.editReply(`No tengo permisos para enviar mensajes aqui`);
        ...
        const embed = new MessageEmbed()
            ...
            .setFooter({
                text: `Poll ID : ${idPoll} | 0 votes | ${this.client.user?.username} Bot v${
                    (this.client as Client).version
                }`,
