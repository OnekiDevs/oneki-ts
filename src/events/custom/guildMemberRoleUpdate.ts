...
export default async function ({ server, oldMember, newMember }: GuildMemberOptions) {
    ...
    if (wasAdded)
        return userActivitieChannel.send({
            content: server.translate('useractivitie_event.roles_change.added', {
    ...
    return userActivitieChannel.send({
        content: server.translate('useractivitie_event.roles_change.removed', {
            user: newMember.user.toString(),
            role: changedRole.toString()
        }),
        allowedMentions: { roles: [] }
